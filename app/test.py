...
class App(tk.Tk):
    ...
    def make_widgets(self):
        ...
        self.canvas_origin.get_tk_widget().grid(row=0, column=0,rowspan=6)
        ...
        self.canvas_result.get_tk_widget().grid(row=0, column=2,rowspan=6)
        self.figure_result.tight_layout()
        self.canvas_result.draw()

        # 右侧按钮
        self.button_open = tk.Button(self, text='打开图片', command=self.open_file)
        self.button_open.grid(row=1, column=1, padx=10, pady=10)

        self.button_save = tk.Button(self, text='保存结果', command=self.save_file)
        self.button_save.grid(row=3, column=1, padx=10, pady=10)

        self.button_open2 = tk.Button(self, text='打开图片2', command=self.open_file2)
        self.button_open2.grid(row=5, column=1, padx=10, pady=10)
    ...
    def imageBinarizationAdaptive(self, source_path):
        _, ext = os.path.splitext(source_path)
        output_path = os.path.join(os.path.dirname(source_path),'output.' + ext)
        # img = cv2.imread(source_path, 0)
        img = self.cv_imread(source_path)               #    读取图片
        img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)     #    灰度化
        alpha = 0.05
        win = 21
        img_blur = cv2.GaussianBlur(img, (win, win), 5)
        img2 = np.uint8(img > (1 - alpha) * img_blur) * 255
        # cv2.imwrite(output_path, img2)                #    保存图片
        self.cv_imwrite(output_path, img2)             #    保存图片
        return output_path
    ...
    def open_file(self):
        ...
        if file_path:
            ...
            output_path = self.imageBinarizationAdaptive(file_path)
            self.result = self.cv_imread(output_path)
            self.show_image()
